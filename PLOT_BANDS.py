...
file_MU = open('Data/mu.dat','r')
mu = np.loadtxt(file_MU)
file_MU.close()

file_BANDS = open('Data/bands.dat','r')
MAT_BANDS = np.loadtxt(file_BANDS)-mu
file_BANDS.close()

file = open('Data/EQ_BC_LOOP_PATH.dat','r')
...
aboveM = MAT_BANDS[lengthKPath,:][MAT_BANDS[lengthKPath,:] > 0.0].min()
belowM = MAT_BANDS[lengthKPath,:][MAT_BANDS[lengthKPath,:] < 0.0].max()
GAP_M = aboveM-belowM
...
ax12.plot(lengthKPath, aboveM, 'bo', linewidth=2.0)
ax12.plot(lengthKPath, belowM, 'bo', linewidth=2.0)
...
ax12.annotate("", xy=(lengthKPath, belowM), xytext=(lengthKPath, aboveM), arrowprops=dict(arrowstyle="<->", color='b'))
